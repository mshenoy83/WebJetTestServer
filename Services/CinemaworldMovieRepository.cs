...
{
    public class CinemaworldMovieRepository : IMovieRepository
    {
        private readonly IHttpClientFactory _httpClientFactory;

        public CinemaworldMovieRepository(IHttpClientFactory httpClientFactory)
        {
        ...
        public async Task<MovieDetailsDto> GetMovieDetails(string Id)
        {
            try
            {
                var cinemaWorlClient = _httpClientFactory.CreateClient("CinemaWorldClient");
                var response = await cinemaWorlClient.GetAsync("movie/cw"+Id);
